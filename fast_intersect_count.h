...
static inline
uint64_t popcount64_unrolled_intersect(const uint64_t* FIC_RESTRICT data1, 
                                       const uint64_t* FIC_RESTRICT data2, 
                                       uint64_t size)
{
    ...
    for (/**/;i < limit; i += 4) {
        cnt += FIC_POPCOUNT(data1[i+0] & data2[i+0]);
        cnt += FIC_POPCOUNT(data1[i+1] & data2[i+1]);
        cnt += FIC_POPCOUNT(data1[i+2] & data2[i+2]);
        cnt += FIC_POPCOUNT(data1[i+3] & data2[i+3]);
    }

    for (/**/;i < size; ++i)
        cnt += FIC_POPCOUNT(data1[i] & data2[i]);
...
#endif
static
uint64_t popcnt_avx2_lookup_intersect(const uint8_t* FIC_RESTRICT data1, 
                                      const uint8_t* FIC_RESTRICT data2, 
                                      const size_t n)
{
...
#define ITER { \
        const __m256i vec = _mm256_and_si256(_mm256_loadu_si256(reinterpret_cast<const __m256i*>(data1 + i)), _mm256_loadu_si256(reinterpret_cast<const __m256i*>(data2 + i))); \
                                       ...
                                       const uint32_t n_ints)
{
  ...
  __attribute__ ((target ("avx2")))
#endif
static uint64_t intersect_bitmaps_avx2_lookup(const uint64_t* FIC_RESTRICT b1, 
                                              const uint64_t* FIC_RESTRICT b2, 
                                              const uint32_t n_ints)
{
    uint64_t count = 0;
    ...
    count += popcnt_avx2_lookup_intersect((uint8_t*)b1, (uint8_t*)b2, n_ints*8);
...
uint64_t intersect_bitmaps_scalar(const uint64_t* FIC_RESTRICT b1, const uint64_t* FIC_RESTRICT b2, const uint32_t n_ints) {
    return popcount64_unrolled_intersect(b1, b2, n_ints);
}
...
uint64_t intersect(const void* data, const uint32_t n_vectors, const uint32_t n_bitmaps_vector) {
    ...
    if ((cpuid & bit_AVX512BW) && n_bitmaps_vector >= 128) { // 16*512
        return c_fwrapper_blocked(n_vectors, 
            (uint64_t*)data, 
            n_bitmaps_vector, 
    ...
    if ((cpuid & bit_AVX2) && n_bitmaps_vector >= 4) {
        return c_fwrapper_blocked(n_vectors, 
            (uint64_t*)data, 
            n_bitmaps_vector, 
            &intersect_bitmaps_avx2_lookup, 
            FIC_DEFAULT_BLOCK/(n_bitmaps_vector*8));
    }
#endif

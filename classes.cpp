...
// c
uint64_t intersect_vector16_cardinality(const uint16_t* TWK_RESTRICT v1, 
...
uint64_t intersect_vector32_unsafe(const uint32_t* TWK_RESTRICT v1, 
...
uint32_t TWK_bitmap_serialized_size(TWK_bitmap_t* bitmap) {
    ...
    // printf("Bitmap size=%u. bitmap=%u, scalar=%u\n",total,bitmap->n_bitmap,bitmap->n_scalar);
    return total;
}
...
TWK_bitmap_t* TWK_bitmap_new() {
    ...
    all->data = NULL;
    // memset(all->data, 0, n_bitmap*sizeof(uint64_t));
    all->scalar = NULL;
    all->n_bitmap = 0;
    all->own_data = 1;
    all->own_scalar = 1;
...
int TWK_bitmap_add_with_scalar(TWK_bitmap_t* bitmap, const uint32_t* values, const uint32_t n_values) {
    ...
    if (bitmap->scalar == NULL) {
        ...
        // bitmap->scalar = (uint16_t*)malloc(new_m*sizeof(uint16_t));
        uint32_t alignment = TWK_get_alignment();
    ...
    if (bitmap->n_scalar >= bitmap->m_scalar) {
        ...
        bitmap->m_scalar = new_m;
        // bitmap->scalar = (uint16_t*)realloc(bitmap->scalar, bitmap->m_scalar * sizeof(uint16_t));
    ...
    for (int i = 0; i < n_values; ++i) {
        ...
        // printf("value=%u adjusted=%u blockid=%u\n",values[i],v,bitmap->id);

        // printf("%u\n",bitmap->data[v/64]);
        int is_unique = (bitmap->data[v / 64] & 1ULL << (v % 64)) == 0;
        bitmap->data[v / 64] |= 1ULL << (v % 64);
        // std::cerr << std::bitset<64>(bitmap->data[v/64]) << std::endl;
...
uint64_t TWK_bitmap_intersect_cardinality(TWK_bitmap_t* TWK_RESTRICT bitmap1, TWK_bitmap_t* TWK_RESTRICT bitmap2) {
    ...
    if (bitmap1->n_bitmap == 0 && bitmap2->n_bitmap == 0) {
        ...
        printf("scalar-scalar (%u<>%u)\n",bitmap1->n_scalar,bitmap2->n_scalar);
        return intersect_vector16_cardinality(bitmap1->scalar, bitmap2->scalar, bitmap1->n_scalar, bitmap2->n_scalar);
        
    } else if (bitmap1->n_bitmap && bitmap2->n_bitmap == 0) {
        // bitmap-scalar comparison
        printf("bitmap-scalar\n");
        // this is list
        uint64_t count = 0;
...
uint64_t TWK_bitmap_cont_intersect_cardinality(const TWK_bitmap_cont_t* TWK_RESTRICT bitmap1, const TWK_bitmap_cont_t* TWK_RESTRICT bitmap2) {
    ...
    // bitmap1->block_ids[bitmap1->n_bitmaps]
...
int TWK_cont_add(TWK_cont_t* bitmap, const uint32_t* values, const uint32_t n_values) {
    ...
    // if (pos > bitmap->m_conts) {
    //     bitmap->m_conts = pos + 1024;
    ...
    // if (pos > bitmap->n_conts) bitmap->n_conts = pos;
